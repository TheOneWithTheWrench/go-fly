...
func TestGitHubFetcherFetchAll(t *testing.T) {
	newSut := func(runner internal.Runner) *remote.GitHubFetcher { return remote.NewGitHubFetcher(runner) }
		...
		var (
			...
			sut    = newSut(internal.RunnerFunc(func(ctx context.Context, name string, args ...string) ([]byte, error) {
				...
				responses := map[string][]byte{
					...
					"user/repos":      []byte(`[{"name":"repo","full_name":"user/repo","ssh_url":"git@github.com:user/repo.git"},{"name":"shared","full_name":"acme/shared","ssh_url":"git@github.com:acme/shared.git"}]`),
					"orgs/acme/repos": []byte(`[{"name":"shared","full_name":"acme/shared","ssh_url":"git@github.com:acme/shared.git"},{"name":"tools","full_name":"acme/tools","ssh_url":"git@github.com:acme/tools.git"}]`),
				}
